import { getTeamByName, getStadiumById } from "./store.js";

export function renderTeamDetail(teamName) {
  const root = document.querySelector("#team-detail .content") || document.querySelector("#team-detail");
  ...
  root.innerHTML = `
    ...
    <ul>
      ...
      <li><strong>Location:</strong> ${st?.location?.city ?? ""}${st?.location ? ", " + st.location.state : ""}</li>
